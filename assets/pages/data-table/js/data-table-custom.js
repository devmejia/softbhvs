...
$(document).ready(function(){$('#dt-live-dom').DataTable({"columns":[null,{"orderDataType":"dom-text-numeric"},{"orderDataType":"dom-text",type:'string'},{"orderDataType":"dom-select"}]});});
$("#dt-server-processing").DataTable({
    processing: true,
    serverSide: true,
    ajax: "dt-json-data/scripts/server-processing.php",
    columns: [{ data: "first_name" }, { data: "last_name" }, { data: "position" }, { data: "office" }, { data: "start_date" }, { data: "salary" }],
});
$("#dt-http").DataTable({
    processing: true,
    serverSide: true,
...
$("#dt-post").DataTable({
    processing: true,
    serverSide: true,
    ajax: { url: "dt-json-data/scripts/post.php", type: "post" },
    columns: [{ data: "first_name" }, { data: "last_name" }, { data: "position" }, { data: "office" }, { data: "start_date" }, { data: "salary" }],
});
$("#dt-ajax-array").DataTable({ ajax: "dt-json-data/arrays.txt" });
$("#dt-ajax-object").DataTable({ ajax: "dt-json-data/objects.txt", columns: [{ data: "name" }, { data: "position" }, { data: "office" }, { data: "extn" }, { data: "start_date" }, { data: "salary" }] });
$("#dt-nested-object").DataTable({
    processing: true,
    ajax: "dt-json-data/objects_deep.txt",
    columns: [{ data: "name" }, { data: "hr.position" }, { data: "contact.0" }, { data: "contact.1" }, { data: "hr.start_date" }, { data: "hr.salary" }],
});
$("#dt-orthogonal").DataTable({
    ajax: "dt-json-data/orthogonal.txt",
    columns: [{ data: "name" }, { data: "position" }, { data: "office" }, { data: "extn" }, { data: { _: "start_date.display", sort: "start_date.timestamp" } }, { data: "salary" }],
});
var generatetable = $("#dt-generate-content").DataTable({ ajax: "dt-json-data/arrays.txt", columnDefs: [{ targets: -1, data: null, defaultContent: "<button>Click!</button>" }] });
$("#dt-generate-content tbody").on("click", "button", function () {
    var data = generatetable.row($(this).parents("tr")).data();
    alert(data[0] + "'s salary is: " + data[5]);
});
$("#dt-render").DataTable({ ajax: "dt-json-data/arrays.txt", deferRender: true });
$("#dom-table").DataTable();
$("#ajax-table").DataTable({ ajax: "dt-json-data/ajax-table.json" });
var dataSet = [
...
$("#jsource-table").DataTable({ data: dataSet, columns: [{ title: "Name" }, { title: "Position" }, { title: "Office" }, { title: "Extn." }, { title: "Start date" }, { title: "Salary" }] });
$("#server-table").DataTable({ processing: true, serverSide: true, ajax: "dt-json-data/server-table.php" });
